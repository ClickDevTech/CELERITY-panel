...
Установщик скачивает пробер и ядро, один раз проходит регистрацию и ставит службу (systemd, procd, launchd или служба Windows). Токен регистрации передаётся через окружение, потому что таблица процессов доступна другим локальным пользователям.
...
| OpenWRT | procd, работает от root | `/etc/celerity-probe` |
...
На роутере установщику нужен `curl` (`opkg update && opkg install curl`); он берёт MIPS-сборку без FPU и определяет порядок байт по `DISTRIB_ARCH`, потому что OpenWRT для обоих вариантов сообщает `mips`. Каталог данных лежит в `/etc`: `/var` там — симлинк на tmpfs, и токен пробера не пережил бы перезагрузку. Логи: `logread -f -e celerity-probe`.

**Место.** Ядро распаковывается примерно в 86 МБ, поэтому там, где лежит каталог данных, нужно около 160 МБ свободного места — больше, чем внутренняя флешка типичного роутера на 16/32 МБ. Укажите внешний накопитель, и установщик будет распаковывать туда же:
...
Свободное место проверяется до первой загрузки: установщик назовёт требуемый объём, а не забьёт overlay на середине.
...
- **CDN-ноды.** CDN-нода проверяется через CDN, а не по origin: каждый включённый edge-адрес — отдельный инбаунд со своей историей, поэтому один мёртвый edge виден сразу, а не растворяется в среднем. У origin остаётся собственная запись, так что одновременное падение всех edge'ей при живом origin указывает на CDN, а не на сервер.
