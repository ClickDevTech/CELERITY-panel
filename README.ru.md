...
## 🛡️ Интеграция wg-easy / awg-easy

Во вкладке **WireGuard** раздела «Ноды» можно подключить один или несколько внешних серверов по IP и порту либо домену. Поддерживаются:

- wg-easy v15 (`логин + пароль`, Basic Auth);
- wg-easy v14 и совместимые awg-easy/amnezia-wg-easy (`только пароль`, cookie-сессия);
- автоматическое определение версии API, проверка соединения и повторная синхронизация.

Для каждого нового пользователя CELERITY создаёт отдельного клиента во всех включённых подключениях WireGuard и сохраняет его `.conf` в зашифрованном виде. Профиль доступен в карточке пользователя и на его странице подписки. Отключение, повторное включение, истечение срока и удаление пользователя синхронизируются с внешними серверами.

> Basic Auth в wg-easy v15 не работает при включённом 2FA у учётной записи API. Используйте отдельную учётную запись без 2FA или отключите 2FA для неё.
